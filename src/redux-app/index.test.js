...
import { setName, place, clearBoard } from "./actions";

describe("reducer", () => {
  const afterTwo = ["Abcd", "Efgh"].map(setName).reduce(reducer, initialState);

  describe("setName(name)", () => {
    it("sets first name if none present", () => {
      const result = reducer(initialState, setName("Abcd"));
      expect(result.names).toEqual(["Abcd"]);
      expect(result.wins).toBeUndefined();
    });
    ...
    it("does nothing if two names present", () => {
      const afterThird = reducer(afterTwo, setName("Ijkl"));
      expect(afterTwo).toEqual(afterThird);
    });
  });
